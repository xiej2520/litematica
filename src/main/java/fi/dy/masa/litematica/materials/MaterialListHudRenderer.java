...
public class MaterialListHudRenderer implements IInfoHudRenderer
{
    ...
    public static void renderLookedAtBlockInInventory(ContainerScreen<?> gui, MinecraftClient mc)
    {
    ...
    public static void highlightSlotsWithItem(ItemStack referenceItem, ContainerScreen<?> gui, Color4f color, MinecraftClient mc)
    {
        List<Slot> slots = gui.getContainer().slots;
        ...
        for (Slot slot : slots)
        {
            ...
            {
                renderOutlinedBox(guiX + slot.xPosition, guiY + slot.yPosition, 16, 16, color.intValue, color.intValue | 0xFF000000, 1f);
            }
        }
